...
def process_fgad(clients, server, args):
    ...
    if args.eval == True:
        ...
        for client in clients:
            # print(torch.load("./weight/"+args.data_group+"/Client_"+str(idx)+"_[0.6685].pth").keys())
            client.shared_GIN.load_state_dict(torch.load("./weight/"+args.data_group+"/Client_"+str(idx)+"_[0.6635].pth")['shared_GIN'])
            client.stu_model.load_state_dict(torch.load("./weight/"+args.data_group+"/Client_"+str(idx)+"_[0.6635].pth")['stu_head'])
            idx+=1
        client_AUC=[]
        client_AUCPR = []
        for client in clients:
